...
static const struct sc_option options[] = {
    ...
    },
    {
        ...
        .longopt = "linkandroid-preview-interval",
        .argdesc = "ms",
        .text = "Capture and send video frame screenshots to WebSocket\n"
    ...
    },
    {
        ...
        .longopt = "linkandroid-skip-taskbar",
        .text = "Hide the application icon from the taskbar (Windows)\n"
                "or Dock (macOS). Useful for running in background.",
    },
};
...
bool sc_parse_shortcut_mods(const char *s, uint8_t *mods)
{
                       ...
                       const char *optstring, const struct option *longopts)
{
    ...
    while ((c = getopt_long(argc, argv, optstring, longopts, NULL)) != -1)
    {
        ...
        case OPT_LINKANDROID_PANEL_SHOW:
            ...
            break;
        case OPT_LINKANDROID_PREVIEW_INTERVAL:
        {
            char *endptr;
            long ms = strtol(optarg, &endptr, 10);
            if (*endptr != '\0' || ms < 0)
            {
                LOGE("Invalid preview interval: %s", optarg);
                return false;
            }
        ...
        case OPT_LINKANDROID_SKIP_TASKBAR:
            opts->linkandroid_skip_taskbar = true;
            break;
        default:
    ...
    // LinkAndroid: Video is needed if preview sender is enabled
    bool needs_video_for_preview = opts->linkandroid_server && opts->linkandroid_preview_interval > 0;

    if (opts->video && !opts->video_playback && !opts->record_filename && !v4l2 && !needs_video_for_preview)
    {
    ...
    if (opts->control)
    {
        ...
        else if (opts->keyboard_input_mode == SC_KEYBOARD_INPUT_MODE_UHID_OR_AOA)
        {
            opts->keyboard_input_mode = otg ? SC_KEYBOARD_INPUT_MODE_AOA
        ...
        else if (opts->mouse_input_mode == SC_MOUSE_INPUT_MODE_SDK && !opts->video_playback)
        {
    ...
    if (otg)
    {
        ...
        if (gmode != SC_GAMEPAD_INPUT_MODE_AOA && gmode != SC_GAMEPAD_INPUT_MODE_DISABLED)
        {
            LOGE("In OTG mode, --gamepad only supports aoa or disabled.");
            return false;
        }

        if (kmode == SC_KEYBOARD_INPUT_MODE_DISABLED && mmode == SC_MOUSE_INPUT_MODE_DISABLED && gmode == SC_GAMEPAD_INPUT_MODE_DISABLED)
        {
    ...
    if (opts->mouse_input_mode != SC_MOUSE_INPUT_MODE_SDK && !opts->mouse_hover)
    {
    ...
    if (opts->display_ime_policy != SC_DISPLAY_IME_POLICY_UNDEFINED && opts->display_id == 0 && !opts->new_display)
    {
